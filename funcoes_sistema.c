...
static COORDENADA direcaoMovimento(int linha_atual, int coluna_atual, int linhas, int colunas, ENTIDADE **matriz);
static int moverLobo(int l, int c, int linhas, int colunas, ENTIDADE **matriz, ENTIDADE **matriz_futura);
static int moverOvelha(int l, int c, int linhas, int colunas, ENTIDADE **matriz, ENTIDADE **matriz_futura);
static int moverVazio(int l, int c, int linhas, int colunas, ENTIDADE **matriz, ENTIDADE **matriz_futura);

// Função direcaoMovimento:
COORDENADA direcaoMovimento(int linha_atual, int coluna_atual, int linhas, int colunas, ENTIDADE **matriz) { 
    /*
    Está função define uma direção (NORTE, SUL, LESTE e OESTE) aleatoriamente.
    OBS: Estou passando como parametro a matriz as linhas e colunas para que futuramente está função sirva de escopo para a IA básica
...
void limparMatriz(ENTIDADE **matriz, int linhas, int colunas) { // Inicio limparMatriz.
    /*
...
void gerarEntidades (ENTIDADE **matriz, int linhas, int colunas, TIPO_ENTIDADE ent, int quant_ent) {
    ...
    */
    ...
    while (quant_ent > cont_ent) { // Inicio do loop.
        ...
        linha_sorteada = rand() % linhas;
        coluna_sorteada = rand() % colunas;
...
// Mover Ovelha:
int moverOvelha (int l, int c, int linhas, int colunas, ENTIDADE **matriz, ENTIDADE **matriz_futura) { // Inicio moverOvelha.
    /*
    ...
    */
    ...
    while (andou && tentativas < 4) {
        ...
        COORDENADA direcao = direcaoMovimento(l, c, linhas, colunas, matriz);
        // Checo se o movimento é valido:
        if (direcao.l >= 0 && direcao.l < linhas && direcao.c >= 0 && direcao.c < colunas) { 
...
// Mover Lobo:
int moverLobo (int l, int c, int linhas, int colunas, ENTIDADE **matriz, ENTIDADE **matriz_futura) { // Inicio moverLobo.
    /*
...
// moverVazio:
int moverVazio (int l, int c, int linhas, int colunas, ENTIDADE **matriz, ENTIDADE **matriz_futura) {
...
int processarTurno(TIPO_ENTIDADE qual_entidade, int linhas, int colunas, ENTIDADE **matriz, ENTIDADE **matriz_futura) {
    ...
    for (int i = 0; i < linhas; i++) {
        for (int j = 0; j < colunas; j++) {
            // Quando ele encontra a entidade atual:
            if (matriz[i][j].tipo == qual_entidade) {
                // Verifica qual é a entidade e sua respectiva função e adiciona a pontuação:
                pontos_turno += tabelaComportamentos[qual_entidade](i, j, linhas, colunas, matriz, matriz_futura);
            }
        }
    }
    return pontos_turno;
